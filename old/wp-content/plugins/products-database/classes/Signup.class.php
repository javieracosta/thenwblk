 ...
 *
 * @package    WordPress
 * @subpackage Products Database Plugin
...
class Signup {
	...
	public function __construct( $params ) {
		...
		$this->signup_columns = Products_db::get_column_atts( 'signup' );

		$this->options = get_option( Products_Db::$products_db_options );
		...
		$this->email_header = Products_Db::$email_headers;
      ...
      $this->groups = Products_Db::get_groups( '`name`,`title`,`description`' );
		...
		if ( false === $submission_id ) {
				...
				if (
						( 
						 ! isset( $_GET['pid'] )
						 ||
						 ( isset( $_GET['pid'] ) && false === Products_Db::get_product_id( $_GET['pid'] ) ) 
						)
						&&
						$atts['type'] != 'thanks'
					 )
			...
			$this->_load_product( $submission_id );
			
			
			$this->registration_page = Products_Db::get_record_link( $this->product['private_id'] );
	...
	private function _form( $atts ) {
		...
		$product_values = Products_Db::get_product( Products_Db::$id_base_number );
		...
		if ( is_object( Products_Db::$validation_errors ) ) {
			...
			<form method="post" enctype="multipart/form-data" >
				<?php
				FormElement::print_hidden_fields( array( 'action'=>'signup', 'source'=>Products_Db::PLUGIN_NAME, 'shortcode_page' => basename( $_SERVER['REQUEST_URI'] ), 'thanks_page' => $this->submission_page ) );
				?>
			...
			foreach ( $this->signup_columns as $column ) :
				...
				if ( in_array( $column->form_element, array( 'hidden' ) ) ) {
					...
					$this->_print_hidden_field( $column, $product_values );
  ...
  private function _print_group_row( $group ) {
	...
	private function _do_receipt() {
		...
		if ( ! isset( $this->product['email'] ) || empty( $this->product['email'] ) ) return NULL;

		$body = $this->_proc_tags( $this->receipt_body );

		$this->_mail( $this->product['email'], $this->receipt_subject, $body );
	...
	private function _proc_tags( $text, $values = array(), $tags = array() ) {
		...
		if ( empty( $values ) ) {
			...
			foreach( $this->signup_columns as $column ) {
				...
				$values[] = Products_Db::prep_field_for_display( $this->product[$column->name], $column->form_element );
	...
	// sets up the product property with only the the signup values
	private function _load_product( $id ) {

		$p = Products_Db::get_product( $id );

		foreach( $this->signup_columns as $column ) {

			$this->product[ $column->name ] = $p[ $column->name ];

		}

	}
	
	private function _print_hidden_field( $column, $product_values ) {
		
		// unserialize it if the default is an array
		$value = isset( $product_values[ $column->name ] ) ? $product_values[ $column->name ] : false  ;
