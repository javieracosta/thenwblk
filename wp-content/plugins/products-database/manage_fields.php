...
global $PDb_i18n;
$PDb_i18n = array(
  'update fields'   => __( 'Update Fields', Products_Db::PLUGIN_NAME ),
  'update groups'   => __( 'Update Groups', Products_Db::PLUGIN_NAME ),
  'add field'       => __( 'Add Field',     Products_Db::PLUGIN_NAME ),
  'add group'       => __( 'Add Group',     Products_Db::PLUGIN_NAME ),
	'new field title' => __( 'new field title',     Products_Db::PLUGIN_NAME ),
	'new group title' => __( 'new group title',     Products_Db::PLUGIN_NAME ),
  );
...
if ( isset( $_POST['action'] ) ) {
	...
	switch ( $_POST['action'] ) {
		...
		case 'reorder_fields':
			...
			foreach( $_POST as $key => $value ) {
				$wpdb->update( Products_Db::$fields_table, array( 'order' => $value ), array( 'id' => str_replace( 'row_','',$key ) ) );
			}
			break;
    ...
    case 'reorder_groups':
      unset( $_POST['action'], $_POST['submit'] );
      foreach( $_POST as $key => $value ) {
        $wpdb->update( Products_Db::$groups_table, array( 'order' => $value ), array( 'name' => str_replace( 'order_','',$key ) ) );
      }
      break;
		...
		case $PDb_i18n['update fields']:
            ...
            $sql = "SHOW FIELDS FROM ".Products_Db::$products_table." WHERE field = '".$row['name']."'";
            $field_info = $wpdb->get_results( $sql );
            $new_type = strtolower( Products_Db::set_datatype( $row['form_element'] ) );
            if ( $new_type != current($field_info)->Type ) {

              $sql = "ALTER TABLE ".Products_Db::$products_table." MODIFY COLUMN `".$row['name']."` ".$new_type;
              ...
              $result = $wpdb->get_results( $sql );
					...
					$wpdb->update( Products_Db::$fields_table, $row, array( 'id'=> $id ) );
			...
			Products_Db::update_default_record();
    ...
    case $PDb_i18n['update groups']:
      ...
      foreach( $_POST as $name => $row ) {
          ...
          $wpdb->update( Products_Db::$groups_table, $row, array( 'name'=> $name ) );
      ...
      break;
		...
		case $PDb_i18n['add field']:
			...
			$result = Products_Db::add_blank_field( $atts );
		...
		case $PDb_i18n['add group']:
			...
			$wpdb->insert( Products_Db::$groups_table, $atts );
		...
		case 'delete_field':
			...
			$result = $wpdb->query('
				DELETE FROM '.Products_Db::$fields_table.'
		...
		case 'delete_group':
			...
			$group_count = $wpdb->get_var( $wpdb->prepare( 'SELECT COUNT(*) FROM '.Products_Db::$fields_table.' WHERE `group` = "' . $_POST['delete'].'"' ) );

			if ( $group_count == 0 ) $result = $wpdb->query('
																												DELETE FROM '.Products_Db::$groups_table.'
...
$groups = Products_Db::get_groups( 'name' );
...
foreach( $groups as $group ) {
	...
	$sql = "SELECT $select_columns FROM " . Products_Db::$fields_table . ' WHERE `group` = "'.$group.'" ORDER BY `order` ';
...
<h3><?php _e('Manage Database Fields',Products_Db::PLUGIN_NAME )?></h3>
<?php
if ( ! empty( $error_msgs ) ) :
?>
...
<?php endif?>
<h4><?php _e('Field Groups',Products_Db::PLUGIN_NAME )?>:</h4>
<div id="fields-tabs">
	<ul>
		<?php
		foreach ( $groups as $group ) {
		...
		echo '<li><a href="#field_groups">'.__('Field Groups',Products_Db::PLUGIN_NAME ).'</a></li>';
    echo '<li><a href="#help">'.__('Help',Products_Db::PLUGIN_NAME ).'</a></li>';
		?>
	</ul>
	<?php
	foreach ( $groups as $group ) :
	...
	<div id="<?php echo $group?>" class="manage-fields-wrap" >
		...
		<h3><?php echo ucwords( str_replace( '_',' ',$group ) ),' ', __('Fields',Products_Db::PLUGIN_NAME )?></h3>
		<p>
		<?php
		if ( 'internal' !== $group ) :
      ...
      <?php if ( 'internal' !== $group ) : ?>
		...
		<tbody id="<?php echo $group?>_fields">
			...
			?>
			<tr><td colspan="<?php echo count( $attribute_columns[ $group ] ) + 1 ?>"><?php _e('No fields in this group',Products_Db::PLUGIN_NAME )?></td></tr>
			<?php
			} else {
        	...
        	<?php if ( 'internal' !== $group ) : ?>
					...
					foreach( array( 'id'/*,'name'*/ ) as $attribute_column ) {
						...
						$value = Products_Db::prepare_value( $database_row[ $attribute_column ] );
	
						$element_atts = array_merge( Products_Db::get_edit_field_type( $attribute_column ),
																					array(
	...
	$groups = Products_Db::get_groups( '`order`,`display`,`name`,`title`,`description`' );
	?>
	<div id="field_groups" class="manage-fields-wrap">
		...
		<h3><?php _e('Edit / Add / Remove Field Groups',Products_Db::PLUGIN_NAME )?></h3>
		<p>
		<?php
		...
		<thead>
			<tr>
				<th scope="col" class="fields vertical-title"><span><?php echo PDb_header( __('fields',Products_Db::PLUGIN_NAME ) ) ?></span></th>
        <th scope="col" class="delete vertical-title"><span><?php echo PDb_header( __('delete',Products_Db::PLUGIN_NAME ) ) ?></span></th>
			<?php
		...
		foreach ( $groups as $group => $group_values ) {
			...
			$group_count = $wpdb->get_var( $wpdb->prepare( 'SELECT COUNT(*) FROM '.Products_Db::$fields_table.' WHERE `group` = "' . $group.'"' ) );
  ...
  global $PDb_i18n;
    ...
    $item = false;
      ...
      case $PDb_i18n['add field']:
        ...
        $message = sprintf(__('The %1$s was not added. Your new %1$s must have a unique name.', Products_Db::PLUGIN_NAME ), $item );
