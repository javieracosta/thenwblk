...
class Pagination 
{
	...
	function create_links()
	{
		...
		if ($loopStart != 1){
			$output .= sprintf('<li class="disabledpage"><a href="' . $link . '">'.__('First',Products_Db::PLUGIN_NAME ).'</a></li>', '1');
		}
		
		if ($currentPage > 1){
			$output .= sprintf('<li class="prevpage"><a href="' . $link . '">'.__('',Products_Db::PLUGIN_NAME ).'</a></li>', $currentPage - 1);
		}
		...
		if ($currentPage < $totalPages){
			$output .= sprintf('<li class="nextpage"><a href="' . $link . '">'.__('',Products_Db::PLUGIN_NAME ).'</a></li>', $currentPage + 1);
		}
		
		if ($loopEnd != $totalPages){
			$output .= sprintf('<li class="nextpage"><a href="' . $link . '">'.__('',Products_Db::PLUGIN_NAME ).'</a></li>', $totalPages);
		}
