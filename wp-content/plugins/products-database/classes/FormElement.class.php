...
class FormElement {
  ...
  public function __construct( $parameters ) {
    ...
    $this->i18n = array(
    					'other' => _x('other', 'indicates a write-in choice', Products_Db::PLUGIN_NAME ),
							'linktext' => _x( 'Link Text','indicates the text to be clicked to go to another web page', Products_Db::PLUGIN_NAME )
    					);
  ...
  private function _hidden() {
	...
	private function _upload() {
		...
		if ( ! empty( $this->value ) ) {
			...
			$this->_addline( '<a href = "'.$this->value.'" target="_blank" ><img class="uploaded-image" style="width:450px" id="image_'.$this->name.'" src="'.$this->value.'" /> </a>' );
			//$this->_addline( '<div>Delete Field</div>');
			
		} else {
      $this->_addline('<img src="http://placehold.it/450x300" />');
    }
