...
class PDb_Init
{
    ...
    private function _activate()
    {
      ...
      $this->_define_init_arrays();
        ...
        $sql = 'CREATE TABLE '.Products_Db::$fields_table.' (
        ...
        $sql = 'CREATE TABLE '.Products_Db::$groups_table.' (
          `id` INT(3) NOT NULL AUTO_INCREMENT,
          `order` INT(3) NOT NULL DEFAULT 0,
        ...
        $sql = 'CREATE TABLE ' . Products_Db::$products_table . ' (
        ...
        foreach( self::${$group.'_fields'} as $name => &$defaults ) {
          ...
          if ( ! isset( $defaults['form_element'] ) ) $defaults['form_element'] = 'text-line';
            ...
            $datatype = Products_Db::set_datatype( $defaults['form_element'] );
        ...
        $sql .= '`date_recorded` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
          ...
          DEFAULT CHARACTER SET utf8
          AUTO_INCREMENT = '.Products_Db::$id_base_number.'
          ;';

        $wpdb->query($sql);

        // save the db version
        add_option( Products_Db::$db_version );
        update_option( Products_Db::$db_version, self::VERSION );
        ...
        foreach( array_keys( self::$field_groups ) as $group ) {
          ...
          foreach( self::${$group.'_fields'} as $name => $defaults ) {
            ...
            $wpdb->insert( Products_Db::$fields_table, $defaults );
        ...
        foreach( self::$field_groups as $group=>$title ) {
          ...
          $wpdb->insert( Products_Db::$groups_table, $defaults );
      ...
      if ( false === get_option( Products_Db::$db_version ) || '0.1' == get_option( Products_Db::$db_version ) ) {
        ...
        $sql = "ALTER TABLE ".Products_Db::$fields_table." ADD COLUMN `display_column` INT(3) DEFAULT 0 AFTER `validation`,";
        ...
        if ( false !== $wpdb->query( $sql ) ) {
          ...
          update_option( Products_Db::$db_version, '0.2' );
      ...
      if ( '0.2' == get_option( Products_Db::$db_version ) ) {
        ...
        $sql = "ALTER TABLE ".Products_Db::$fields_table." MODIFY COLUMN `values` LONGTEXT NULL DEFAULT NULL";

        if ( false !== $wpdb->query( $sql ) ) {

          // set the version number this step brings the db to
          update_option( Products_Db::$db_version, '0.3' );

        }

      }

      if ( '0.3' == get_option( Products_Db::$db_version ) ) {
         ...
         */
				...
				$wpdb->update( Products_Db::$fields_table, array( 'form_element' => 'date' ), array( 'name' => 'when', 'form_element' => 'text-line' ) );
				 
				//
				$date_fields = $wpdb->get_results( 'SELECT f.name FROM '.Products_Db::$fields_table.' f WHERE f.form_element = "date"', ARRAY_A );
        ...
        if ( ! empty( $df_string ) ) :
					...
					$query = '
						...
						SELECT `id`'.$df_string.'
						FROM '.Products_Db::$products_table;
				...
				// set the version number this step brings the db to
				update_option( Products_Db::$db_version, '0.4' );

      }

      if ( '0.4' == get_option( Products_Db::$db_version ) ) {
        ...
        $sql = "ALTER TABLE ".Products_Db::$fields_table." CHANGE COLUMN `import` `CSV` TINYINT(1)";

        if ( false !== $wpdb->query( $sql ) ) {

          // set the version number this step brings the db to
          update_option( Products_Db::$db_version, '0.5' );
			...
			*/
			if ( '0.5' == get_option( Products_Db::$db_version ) && false === Products_Db::get_product() ) {
       ...
       */
			if ( '0.5' == get_option( Products_Db::$db_version ) ) {

        $sql = "SHOW FIELDS FROM ".Products_Db::$products_table." WHERE `field` IN ('date_recorded','date_updated')";
        ...
        foreach ( $field_info as $field ) {
          ...
          if ( $field->Type !== 'TIMESTAMP' ) {
            ...
            if ( false !== $column_definition ) {
              ...
              $sql = "ALTER TABLE ".Products_Db::$products_table." MODIFY COLUMN ".$column_definition;
      ...
      error_log( Products_Db::PLUGIN_NAME.' plugin activated' );
    ...
    private function _deactivate()
    {
				...
				error_log( Products_Db::PLUGIN_NAME.' plugin deactivated' );
    }
    ...
    private function _uninstall()
    {
        ...
        // delete tables
        $sql = 'DROP TABLE `'.Products_Db::$fields_table.'`, `'.Products_Db::$products_table.'`, `'.Products_Db::$groups_table.'`;';
        $wpdb->query( $sql );

        // remove options
        delete_option( Products_Db::$products_db_options );
				delete_option( Products_Db::$db_version );

				// clear transients
				delete_transient( 'pdb_last_record' );
				
        error_log( Products_Db::PLUGIN_NAME.' plugin uninstalled' );
    ...
    public function set_default_record() {
      ...
      return Products_Db::process_form( $default_values, 'insert', true );
