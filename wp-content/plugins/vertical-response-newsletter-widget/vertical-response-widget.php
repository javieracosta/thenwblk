...
function widget_vr($args = false, $echo = true, $atts = false) {
		...
		$defaults = apply_filters('vr_widget_defaults', array(
			...
			'email_input_size' => 20,
			'email_default_value' => 'email address',
			'name_input_size' => 15,
			'div_class' => 'widget_vr',
			'form_class' => 'vr_form',
...
<input type="text" id="email_address" name="email_address" style="color:#aaa;width:100%;float:left;font-size:.9em;font-weight:normal;" value="'.$email_default_value.'"  onfocus="if (this.value == \'Enter your email address\') {this.value = \'\';}" onblur="if (this.value == \'\') {this.value = \'Enter your email address\';}"';
	if($style) { 
...
$output .=  '<input type="submit" style="border:0;background:transparent;width:30%;margin-left:-31%;margin-top:1%;font-size:1.1em;color:#000;" value="'.htmlspecialchars(stripslashes_deep($button)).'"';
if($style) { 
